...
def process_image(image, top_left, bottom_right, percentage_coordinates_list):
...
def main():
    ...
    while True:
        ...
        success, top_left, bottom_right = detect_rectangle(gray_frame, template, threshold=0.33)

        if success:
            # Задайте координаты для каждого квадрата в процентном соотношении (x, y, x_w, y_h)
            percentage_coordinates_list = [
                (38, 24, 30, 11), (38, 35.5, 30, 10.5), (38, 48, 30, 7), (38, 56.4, 30, 5.5),
                (38, 65, 45, 5.5), (38, 73, 20, 6), (68, 75.1, 20, 6.3), (68, 87, 20, 6)
            ]

            # Process the cropped region
            text, card_part = process_image(frame, top_left, bottom_right, percentage_coordinates_list)
            ...
            if "ИДЕНТИФИКАЦИОННАЯ КАРТА" in text:
                ...
                display_percentage_squares(frame, top_left, bottom_right, percentage_coordinates_list)
